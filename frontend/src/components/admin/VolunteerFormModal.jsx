...
import { createVolunteer, updateVolunteer } from '../../api/admin'

export default function VolunteerFormModal({ volunteer, onClose }){
  const [name, setName] = useState(volunteer.volunteer_name || '')
  const [location, setLocation] = useState(volunteer.location || '')
  const [status, setStatus] = useState(volunteer.status || 'free')
  ...
  async function save(){
    ...
    try{
      if(volunteer && volunteer.id) await updateVolunteer(volunteer.id, { volunteer_name: name, location, status })
      else await createVolunteer({ volunteer_name: name, location, status })
      onClose()
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
      <div className="bg-white p-4 rounded w-96">
        <h4 className="font-semibold mb-2">{volunteer.id ? 'Edit Volunteer' : 'Create Volunteer'}</h4>
